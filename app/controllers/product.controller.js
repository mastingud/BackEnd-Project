...
exports.deleteAll =  (req, res) => {
    ...
    .catch(err => {
        res.status(500).send({
            status :false,
            message : err.message || `Some error occured while removing all productss`
        });
    });
};
...
exports.findProductUser = (req, res) => {
    User.hasMany(Product, {foreignKey: 'id'})
    Product.belongsTo(User, {foreignKey: 'userId'})
    Product.findAll({attributes: ['title', sequelize.fn('COUNT', sequelize.col('userId'))], include: [{ model: User, attributes: ['username'] }], 
    group: ['userId','title','description', 'brand']},{raw: true})
    .then( data => {
        const dat = data.map(dt => {
            //tidy up the user data
            return Object.assign(
